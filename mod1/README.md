...
1. ConfigMap を使用する Pod のマニフェストを確認します。  
   ...
   - **Pod のマニフェストにある `env` は `configMapKeyRef` を使用して ConfigMap の値を `name` で指定した環境変数名で取り込んでいます。**
   - **Pod のマニフェストにある `envFrom` は `configMapRef` を使用して ConfigMap の値をそのまま環境変数名で取り込んでいます。**
   ```

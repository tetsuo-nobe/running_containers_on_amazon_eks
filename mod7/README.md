...
1. Service を表示します。-n の後には自分の Namespace を指定します。**出力から EXTERNAL-IP の値をメモしておきます。** 
   ...
   kubectl get service -n student99
   ```
1. Webブラウザで新しいタブを開き、次のように URL を指定して **Top Page** という文字を含んだ Web ページが表示されることを確認します。**アクセス可能になるまで少し待つ必要があります。** 
   ```
   http://<EXTERNAL-IPの値>
   ```
1. AWS マネジメントコンソールで EC2 のページを開き、左側ナビゲーションメニューで **ロードバランシング** - **ロードバランサー** をクリックします。
1. DNS 名がメモしたものと一致するロードバランサーを見つけて選択します。一覧表で[種類]列の値が classic になっていることを確認します。
1. Service を削除します。
   ```
...
1. Pod の IPアドレス を表示します。-n の後には自分の Namespace を指定します。**出力から全ての Pod の IP の値をメモしておきます。** 
   ```
   kubectl get po -n student01 -o wide
   ```
1. Webブラウザで新しいタブを開き、次のように URL を指定して **Top Page** という文字を含んだ Web ページが表示されることを確認します。**アクセス可能になるまで少し待つ必要があります。** 表示されたら、何回か繰り返してアクセスして、表示される AZ の値が切替わることを確認します。
   ...
   ```
1. AWS マネジメントコンソールで EC2 のページを開き、左側ナビゲーションメニューで **ロードバランシング** - **ロードバランサー** をクリックします。
1. DNS 名がメモしたものと一致するロードバランサーを見つけて選択します。一覧表で[状態]列が Active、[種類]列が application になっていることを確認します。
1. ページの下部にある [リスナー] タブをクリックします。
1. [リスナー ID] で [HTTP : 80] のリンクをクリックします。
1. [Rules] タブをクリックします。
1. [Rule 1]の行の中央にある [Then 1.Forward to] の 下に表示されているリンク (例: k8s-student0-myservic-75a7) をクリックします。これにより、ターゲットグループの情報が表示されます。
1. ページ下部にある [Targets] タブをクリックします。[IP Address] に表示されている IPアドレスが、メモしておいた Pod の IPアドレスと同じであることを確認します。
